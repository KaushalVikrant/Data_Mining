...
pathForBaseFile='E:\Semester 2\Data Mining\Assignments\Assignment 2\Ans 4\\Data Wine\\wine.data.txt'
pathForAttributesFile='E:\Semester 2\Data Mining\Assignments\Assignment 2\Ans 4\\Data Wine\\attributes.txt'
pathForClassVariables='E:\Semester 2\Data Mining\Assignments\Assignment 2\Ans 4\\Data Wine\\classVariables.txt'
...
def readClassVariableFile(path):
    f=open(path,'r')
...
def readFile(path,dataList):
    ...
    for data in f:
        ...
        tempList[len(tempList)-1]=classVar
                ...
                for att in Data:
                    if att[features]<=splitAt:          #if value is smaller than split value
                        ...
                        if att[length]==classValue:      # if class variable is main or others for binary split
                            ...
                            leftCountOthers+=1
                    ########################
                        if leftDict.has_key(att[length]):
                            tempDict=(leftDict[att[length]])
                            leftDict[att[length]]=int(tempDict)+1
                        else:
                            leftDict.update({att[length]:1})
                    ####################
                    else:
...
readClassVariableFile(pathForClassVariables)
readFile(pathForBaseFile,dataList)
readAttributesFile(pathForAttributesFile)
